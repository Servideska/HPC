...
[web page](https://www.spec.org/hpc2021/). ZIH's system Taurus (partition `haswell`) is the
reference system denoting a score of 1. The benchmark comes with different workload sizes - tiny,
small, medium, large - and different parallelization models including MPI only, MPI+OMP,
MPI+openACC, MPI+target offloading.
...
- Hint: Partition `ml` is power9 architecture, use the `-e ppc64le` switch when installing
