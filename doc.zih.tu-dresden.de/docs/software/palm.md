...
```bash
cd </palm_installation/>
cp palm_model_system/packages/palm/model/share/config/.palm.config.taurus_[gnu|intel] .
```
...
??? tip "Submit PALM with batch script"
    === submit_gpalm.sh (GCC 2021a)
        ...
        #SBATCH --account=<project>           # account CPU time to Project
