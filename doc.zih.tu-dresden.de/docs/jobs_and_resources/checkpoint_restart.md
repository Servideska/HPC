...
convenient options are:
    * Via `-i` you can specify an interval (in seconds) in which checkpoint files are to be created
      automatically.
...
job. If the job runs into its time limit, the time to write out the checkpoint
